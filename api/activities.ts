...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      // Compatibilidad: si mandas title/description/date (pruebas), lo acepto.
      // Pero el MVP real según tu UI es objective/community/activity_date/activity_time.
      const objective = String(body?.objective ?? body?.title ?? "");
      const community = body?.community ? String(body.community) : null;
      const description = body?.description ? String(body.description) : null;

      const activity_date = body?.date ? String(body.date) : (body?.activity_date ? String(body.activity_date) : null);
      const activity_time = body?.time ? String(body.time) : (body?.activity_time ? String(body.activity_time) : null);
      ...
      // Si quieres guardar "description" también, primero crea columna description y luego inclúyela.
      // Por ahora la dejamos fuera para evitar otro "column does not exist".
