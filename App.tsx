...
async function apiGetPromoters() {
  ...
  return data as { ok: true; items: Promoter[] };
}

async function apiUpsertPromoter(payload: Partial<Promoter> & { id: string }) {
  const r = await fetch(`/api/promoters`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });
  const data = await r.json();
  if (!r.ok) throw new Error(data?.error || "Error PUT /api/promoters");
  return data as { ok: true; item: Promoter };
}

// Mapping DB rows -> UI Activity
...
const App: React.FC = () => {
  ...
  const [userRole, setUserRole] = useState<UserRole>(
    () =>
      (localStorage.getItem(STORAGE_KEYS.AUTH_ROLE) as UserRole) ||
      UserRole.ADMIN
  );
  ...
  /* =========================
     ...
     - Promoters (perfiles)
  ========================= */
  const refreshPromoters = useCallback(async () => {
    try {
      const data = await apiGetPromoters();
      setPromoters(data.items || []);
      localStorage.setItem(STORAGE_KEYS.PROMOTERS, JSON.stringify(data.items || []));
    } catch (e) {
      console.error("refreshPromoters error:", e);
    }
  }, []);
  ...
  const refreshGlobalData = useCallback(async () => {
    try {
      ...
      // También refresca perfiles para que admin vea cambios
  ...
  }, [userRole, currentPromoterId, refreshPromoters]);
  ...
  }, [isAuthenticated, currentPromoterId, userRole, refreshGlobalData]);
    ...
    async (activity: Activity) => {
      try {
        ...
        const payload = {
          ...
          // compatibilidad si tu tabla aún tiene title/description:
  ...
  const handleBulkAddActivities = useCallback(
    async (newActivities: Activity[]) => {
      // Manténlo simple: refresca del server (fuente de verdad)
      await refreshGlobalData();
    },
    [refreshGlobalData]
  );
  ...
  const handleUpdateActivity = useCallback(
    (id: string, updates: Partial<Activity>) => {
      // Sin endpoint PUT/PATCH todavía: actualiza local
      setActivities((prev) => {
  ...
  // Para ProgramModule (Agenda): SIEMPRE lo visible según selector
  ...
  const handleLogin = async (role: UserRole, userId: string) => {
    ...
    // Refresh inmediato: actividades + perfiles
    await refreshGlobalData();
  };
  ...
  if (!isAuthenticated) {
    return (
      <Login
        onLogin={handleLogin}
        users={promoters}
        onUpdateUser={async (id, up) => {
          // Persistir perfil en Neon (para que admin vea los cambios)
          try {
            const merged = { ...promoters.find(p => p.id === id), ...up, id } as any;
            await apiUpsertPromoter(merged);

            // refresca estado global
            await refreshPromoters();
          } catch (e) {
          ...
          <div className="p-4 md:p-8 max-w-7xl mx-auto">
            ...
            {activeView === "user-management" && (
              <UserManagementModule
                users={promoters}
                onAddUser={async (u) => {
                  await apiUpsertPromoter(u as any);
                  await refreshPromoters();
                }}
                onUpdateUser={async (id, up) => {
                  const merged = { ...promoters.find(p => p.id === id), ...up, id } as any;
                ...
                onDeleteUser={(id) => {
                  // Si quieres delete real, se crea API DELETE.
                  setPromoters((p) => p.filter((u) => u.id !== id));
                }}
              />
            )}
            ...
            {activeView === "profile" && currentPromoter && (
              <ProfileModule
                ...
                onUpdateUser={async (id, up) => {
                  const merged = { ...currentPromoter, ...up, id } as any;
                  await apiUpsertPromoter(merged);
                  await refreshPromoters();
                }}
              />
