...
import { Activity, Promoter, ActivityType, ActivityStatus, UserRole, Notification, CommunityContact } from './types';
...
const App: React.FC = () => {
  ...
  // Fix: Defined filteredPromoters to show all if admin or just the current user if promoter
  ...
  const handleLogin = (role: UserRole) => {
    ...
    if (role === UserRole.FIELD_PROMOTER) {
      ...
      sendAdminNotification('Inicio de Sesión', `El gestor ha iniciado sesión. GPS Activo y Rastreable.`, 'USER_LOGIN');
    }
  };
  ...
  const navItems = [
    ...
    { id: 'reports', label: 'IA Resumen', icon: 'fa-chart-pie', adminOnly: true },
    { id: 'admin-custom-reports', label: 'Informes', icon: 'fa-file-invoice-dollar', adminOnly: true },
      ...
      {!isMobile && (
        <aside className="bg-slate-900 text-slate-300 w-64 flex-shrink-0 flex flex-col border-r border-slate-800">
          <div className="p-6 flex items-center gap-3">
            <div className="w-10 h-10 bg-indigo-600 rounded-lg flex items-center justify-center text-white shadow-lg"><i className="fa-solid fa-route text-xl"></i></div>
            <span className="text-xl font-black tracking-tight text-white">PromoterFlow</span>
          </div>
          <nav className="flex-1 px-4 space-y-1 overflow-y-auto no-scrollbar">
            {navItems.map((item) => (
              <button key={item.id} onClick={() => setActiveView(item.id as View)} className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl transition-all ${activeView === item.id ? 'bg-indigo-600 text-white font-bold' : 'hover:bg-slate-800 hover:text-white'}`}>
                <i className={`fa-solid ${item.icon} w-5`}></i><span className="text-sm">{item.label}</span>
              </button>
            ))}
          </nav>
          <div className="p-4 border-t border-slate-800">
            <button onClick={handleLogout} className="w-full bg-slate-800/50 hover:bg-red-900/20 hover:text-red-400 p-3 rounded-xl flex items-center gap-3 text-xs font-bold transition-all"><i className="fa-solid fa-right-from-bracket"></i> Salir</button>
          </div>
        </aside>
      )}
      ...
      <div className="flex-1 flex flex-col h-full relative overflow-hidden">
        <header className="safe-pt bg-white/80 backdrop-blur-md border-b border-slate-200 px-6 py-4 flex justify-between items-center z-40 sticky top-0">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-lg font-bold text-slate-800">{navItems.find(i => i.id === activeView)?.label}</h1>
          </div>
          <div className="flex items-center gap-3">
            ...
            {userRole === UserRole.FIELD_PROMOTER && isMobile && (
            ...
            <div className="h-10 w-10 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 border border-slate-200 relative">
            ...
            {activeView === 'dashboard' && (
              <div className="space-y-6">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <StatCard icon="fa-clipboard-check" color="text-blue-600" bg="bg-blue-50" label="Labores" value={activities.length.toString()} />
          ...
          <nav className="fixed bottom-0 inset-x-0 bg-white/90 backdrop-blur-xl border-t border-slate-200 safe-pb z-50 flex justify-around items-center px-2 py-1">
            {mobileNavItems.map((item) => (
              <button key={item.id} onClick={() => setActiveView(item.id as View)} className={`flex flex-col items-center justify-center w-full py-2 px-1 transition-all ${activeView === item.id ? 'text-indigo-600' : 'text-slate-400'}`}>
                <div className="text-lg mb-1"><i className={`fa-solid ${item.icon}`}></i></div>
      ...
      {isNewActivityModalOpen && (
        <div className="fixed inset-0 z-[100] flex flex-col bg-white overflow-hidden animate-in slide-in-from-bottom duration-300">
          <header className="safe-pt px-6 py-4 flex justify-between items-center border-b border-slate-100">
            <h2 className="text-xl font-bold text-slate-800">Nueva Registro de Labores</h2>
            <button onClick={() => setIsNewActivityModalOpen(false)} className="w-8 h-8 flex items-center justify-center bg-slate-100 rounded-full text-slate-500"><i className="fa-solid fa-xmark"></i></button>
          </header>
          <div className="flex-1 overflow-y-auto p-6 space-y-8 pb-32">
            <div className="space-y-4">
              ...
              <select className="w-full h-12 bg-slate-50 border border-slate-200 rounded-xl px-4 text-sm" value={newActivity.type} onChange={e => setNewActivity({...newActivity, type: e.target.value as any})}><option value="">Seleccione Tipo</option>{Object.values(ActivityType).map(t => <option key={t} value={t}>{t}</option>)}</select>
            ...
            <div className="space-y-4">
              <label className="text-xs font-black text-indigo-500 uppercase tracking-widest block">Contacto Comunitario</label>
              <div className="bg-indigo-50/50 p-4 rounded-2xl border border-indigo-100 space-y-3">
                <input placeholder="Nombre Contacto" className="w-full h-10 bg-white border border-indigo-100 rounded-xl px-4 text-sm" value={newActivity.communityContact?.name} onChange={e => setNewActivity({...newActivity, communityContact: {...newActivity.communityContact!, name: e.target.value}})} />
                <div className="grid grid-cols-2 gap-2">
                   <input placeholder="Teléfono" type="tel" className="w-full h-10 bg-white border border-indigo-100 rounded-xl px-4 text-sm" value={newActivity.communityContact?.phone} onChange={e => setNewActivity({...newActivity, communityContact: {...newActivity.communityContact!, phone: e.target.value}})} />
                   <label className="flex items-center gap-2 bg-white border border-indigo-100 rounded-xl px-4 text-xs font-bold text-slate-600"><input type="checkbox" checked={newActivity.communityContact?.hasWhatsApp} onChange={e => setNewActivity({...newActivity, communityContact: {...newActivity.communityContact!, hasWhatsApp: e.target.checked}})} /> WA</label>
                </div>
                <input placeholder="Comunidad" className="w-full h-10 bg-white border border-indigo-100 rounded-xl px-4 text-sm" value={newActivity.communityContact?.community} onChange={e => setNewActivity({...newActivity, communityContact: {...newActivity.communityContact!, community: e.target.value}})} />
              </div>
            </div>
            <div className="bg-slate-900 text-white p-6 rounded-2xl space-y-4">
               <div className="flex items-center justify-between"><p className="text-xs font-black uppercase">Verificación</p><i className="fa-solid fa-camera"></i></div>
               <div className="w-full h-32 border-2 border-dashed border-slate-700 rounded-xl flex flex-col items-center justify-center text-slate-500"><i className="fa-solid fa-cloud-arrow-up text-2xl mb-2"></i><p className="text-[10px] font-bold">CARGAR FOTOGRAFÍA</p></div>
            </div>
          </div>
          <div className="p-6 bg-white border-t border-slate-100 safe-pb fixed bottom-0 inset-x-0">
            <button onClick={handleCreateActivity} className="w-full h-14 bg-indigo-600 text-white rounded-2xl font-black shadow-xl shadow-indigo-500/30 uppercase">Finalizar Registro</button>
          </div>
        </div>
      )}
    </div>
...
const StatCard = ({ icon, color, bg, label, value }: { icon: string, color: string, bg: string, label: string, value: string }) => (
  <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 flex items-center gap-3">
    <div className={`w-10 h-10 ${bg} ${color} rounded-xl flex items-center justify-center text-lg flex-shrink-0`}><i className={`fa-solid ${icon}`}></i></div>
    <div className="overflow-hidden"><p className="text-slate-400 text-[9px] font-black uppercase tracking-widest truncate">{label}</p><p className="font-black text-slate-800 truncate text-sm">{value}</p></div>
  </div>
);
